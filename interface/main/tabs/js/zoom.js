async function joinMeeting(meetDetails, patientData) {
    try {
        const thing = `/bsemr/interface/zoom/index.php?meetingId=${meetDetails.meeting_id}&meetingPwd=${meetDetails.password}&meetingRole=1&displayName=${patientData.fname || ''} ${patientData.lname || ''}&meetingEmail=${meetDetails.meeting_for_email}`;
        console.log("This is the thing: ", thing);
        parent.navigateTab(`/bsemr/interface/zoom/index.php?meetingId=${meetDetails.meeting_id}&meetingPwd=${meetDetails.password}&meetingRole=1&displayName=${patientData.fname || ''} ${patientData.lname || ''}&meetingEmail=${meetDetails.meeting_for_email}`, 'meet');
        parent.navigateTab(`/bsemr/interface/zoom/index.php?meetingId=${meetDetails.meeting_id}&meetingPwd=${meetDetails.password}&meetingRole=1&displayName=${patientData.fname || ''} ${patientData.lname || ''}&meetingEmail=${meetDetails.meeting_for_email}`, 'meet');
